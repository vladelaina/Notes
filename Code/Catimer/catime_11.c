        ...
        case WM_COMMAND: {
            switch (LOWORD(wp)) {
                case 101:   //Customize  
                    ...
                    // 获取输入内容并转换为整数
                    int input_time = atoi(inputText);

                    // 如果转换后的值为0且输入的字符串不为空，说明输入不合法，设置为默认值
                    if (input_time == 0 && inputText[0] != '\0') {
                        TOTAL_TIME = 25 * 60;  // 设置为默认时间：25分钟
                    } else {
                        TOTAL_TIME = input_time * 60;  // 将输入时间（分钟）转换为秒数
                    }
